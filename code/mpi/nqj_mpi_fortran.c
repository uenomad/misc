...
void mpi_allgather   (M3C void *sb, MI *sc, MI *st, void *rb, MI *rc, MI *rt, MI *comm, MI *ie) { nqjf_allgather(sb, sc, st, rb, rc, rt, comm, ie); }
...
void mpi_allgatherv  (M3C void *sb, MI *sc, MI *st, void *rb, M3C MI *rcs, M3C MI *displs, MI *rt, MI *comm, MI *ie) { nqjf_allgatherv(sb, sc, st, rb, rcs, displs, rt, comm, ie); }
...
void mpi_allreduce   (M3C void *sb, void *rb, MI *count, MI *dt, MI *op, MI *comm, MI *ie) { nqjf_allreduce(sb, rb, count, dt, op, comm, ie); }
...
void mpi_alltoall    (M3C void *sb, MI *sc, MI *st, void *rb, MI *rc, MI *rt, MI *comm, MI *ie) { nqjf_alltoall(sb, sc, st, rb, rc, rt, comm, ie); }
...
void mpi_alltoallv   (M3C void *sb, M3C MI *scs, M3C MI *sdispls, MI *st, void *rb, M3C MI *rcs, M3C MI *rdispls, MI *rt, MI *comm, MI *ie) { nqjf_alltoallv(sb, scs, sdispls, st, rb, rcs, rdispls, rt, comm, ie); }
...
void mpi_barrier     (MI *comm, MI *ie) { nqjf_barrier(comm, ie); }
...
void mpi_bcast       (void *buffer, MI *count, MI *dt, MI *root, MI *comm, MI *ie) { nqjf_bcast(buffer, count, dt, root, comm, ie); }
...
void mpi_bsend       (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *ie) { nqjf_bsend(buf, count, dt, dest, tag, comm, ie); }
...
void mpi_bsend_init  (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_bsend_init(buf, count, dt, dest, tag, comm, rq, ie); }
...
void mpi_cancel      (MI *rq, MI *ie) { nqjf_cancel(rq, ie); }
void mpi_cancel_     (MI *rq, MI *ie) __attribute__((alias("mpi_cancel")));
void mpi_cancel__    (MI *rq, MI *ie) __attribute__((alias("mpi_cancel")));
void mpi_cart_create  (MI *comm_old, MI *ndims, M3C MI *dims, M3C MI *periods, MI *reorder, MI *comm_cart, MI *ie) { nqjf_cart_create(comm_old, ndims, dims, periods, reorder, comm_cart, ie); }
...
void mpi_cart_sub    (MI *comm, M3C MI *remain_dims, MI *comm_new, MI *ie) { nqjf_cart_sub(comm, remain_dims, comm_new, ie); }
...
void mpi_comm_create  (MI *comm, MI *group, MI *newcomm, MI *ie) { nqjf_comm_create(comm, group, newcomm, ie); }
...
void mpi_comm_dup    (MI *comm, MI *newcomm, MI *ie) { nqjf_comm_dup(comm, newcomm, ie); }
...
void mpi_comm_free   (MI *comm, MI *ie) { nqjf_comm_free(comm, ie); }
...
void mpi_comm_rank   (MI *comm, MI *rank, MI *ie) { nqjf_comm_rank(comm, rank, ie); }
...
void mpi_comm_size   (MI *comm, MI *size, MI *ie) { nqjf_comm_size(comm, size, ie); }
...
void mpi_comm_spawn  (M3C char *command, char *argv, MI *maxprocs, MI *info, MI *root, MI *comm, MI *intercomm, MI *ae, MI *ie) { nqjf_comm_spawn(command, argv, maxprocs, info, root, comm, intercomm, ae, ie); }
...
void mpi_comm_spawn_multiple  (MI *count, char *ac, char *aa, M3C MI *am, M3C MI *array_of_info, MI *root, MI *comm, MI *intercomm, MI *ae, MI *ie) { nqjf_comm_spawn_multiple(count, ac, aa, am, array_of_info, root, comm, intercomm, ae, ie); }
...
void mpi_comm_split  (MI *comm, MI *color, MI *key, MI *newcomm, MI *ie) { nqjf_comm_split(comm, color, key, newcomm, ie); }
...
void mpi_file_close  (MF *fh, MI *ie) { nqjf_file_close(fh, ie); }
...
void mpi_file_read   (MF *fh, void *buf, MI *count, MI *dt, MS *status, MI *ie) { nqjf_file_read(fh, buf, count, dt, status, ie); }
...
void mpi_file_read_all  (MF *fh, void *buf, MI *count, MI *dt, MS *status, MI *ie) { nqjf_file_read_all(fh, buf, count, dt, status, ie); }
...
void mpi_file_read_at  (MF *fh, MPI_Offset *offset, void* buf, MI *count, MI *dt, MS *status, MI *ie) { nqjf_file_read_at(fh, offset, buf, count, dt, status, ie); }
...
void mpi_file_read_at_all  (MF *fh, MPI_Offset *offset, void* buf, MI *count, MI *dt, MS *status, MI *ie) { nqjf_file_read_at_all(fh, offset, buf, count, dt, status, ie); }
...
void mpi_file_write  (MF *fh, M3C void *buf, MI *count, MI *dt, MS *status, MI *ie) { nqjf_file_write(fh, buf, count, dt, status, ie); }
...
void mpi_file_write_all  (MF *fh, M3C void *buf, MI *count, MI *dt, MS *status, MI *ie) { nqjf_file_write_all(fh, buf, count, dt, status, ie); }
...
void mpi_file_write_at  (MF *fh, MPI_Offset *offset, M3C void *buf, MI *count, MI *dt, MS *status, MI *ie) { nqjf_file_write_at(fh, offset, buf, count, dt, status, ie); }
...
void mpi_file_write_at_all  (MF *fh, MPI_Offset *offset, M3C void *buf, MI *count, MI *dt, MS *status, MI *ie) { nqjf_file_write_at_all(fh, offset, buf, count, dt, status, ie); }
...
void mpi_finalize    (MI *ie) { nqjf_finalize(ie); }
void mpi_finalize_   (MI *ie) __attribute__((alias("mpi_finalize")));
void mpi_finalize__  (MI *ie) __attribute__((alias("mpi_finalize")));
void mpi_gather      (M3C void *sb, MI *sc, MI *st, void *rb, MI *rc, MI *rt, MI *root, MI *comm, MI *ie) { nqjf_gather(sb, sc, st, rb, rc, rt, root, comm, ie); }
...
void mpi_gatherv     (M3C void *sb, MI *sc, MI *st, void *rb, M3C MI *rcs, M3C MI *displs, MI *rt, MI *root, MI *comm, MI *ie) { nqjf_gatherv(sb, sc, st, rb, rcs, displs, rt, root, comm, ie); }
...
void mpi_get         (MI *origin_addr, MI *origin_count, MI *origin_dt, MI *target_rank, MI *target_disp, MI *target_count, MI *target_dt, MI *win, MI *ie) { nqjf_get(origin_addr, origin_count, origin_dt, target_rank, target_disp, target_count, target_dt, win, ie); }
...
void mpi_ibsend      (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_ibsend(buf, count, dt, dest, tag, comm, rq, ie); }
...
void mpi_init        (MI *ie) { nqjf_init(ie); }
void mpi_init_       (MI *ie) __attribute__((alias("mpi_init")));
void mpi_init__      (MI *ie) __attribute__((alias("mpi_init")));
void mpi_init_thread  (MI *required, MI *provided, MI *ie) { nqjf_init_thread(required, provided, ie); }
...
void mpi_intercomm_create  (MI *local_comm, MI *local_leader, MI *peer_comm, MI *remote_leader, MI *tag, MI *newintercomm, MI *ie) { nqjf_intercomm_create(local_comm, local_leader, peer_comm, remote_leader, tag, newintercomm, ie); }
...
void mpi_intercomm_merge  (MI *intercomm, MI *high, MI *newintracomm, MI *ie) { nqjf_intercomm_merge(intercomm, high, newintracomm, ie); }
...
void mpi_iprobe      (MI *source, MI *tag, MI *comm, MI *flag, MI *status, MI *ie) { nqjf_iprobe(source, tag, comm, flag, status, ie); }
...
void mpi_irecv       (void *buf, MI *count, MI *dt, MI *source, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_irecv(buf, count, dt, source, tag, comm, rq, ie); }
...
void mpi_irsend      (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_irsend(buf, count, dt, dest, tag, comm, rq, ie); }
...
void mpi_isend       (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_isend(buf, count, dt, dest, tag, comm, rq, ie); }
...
void mpi_issend      (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_issend(buf, count, dt, dest, tag, comm, rq, ie); }
...
void mpi_probe       (MI *source, MI *tag, MI *comm, MI *status, MI *ie) { nqjf_probe(source, tag, comm, status, ie); }
...
void mpi_put         (M3C void *origin_addr, MI *origin_count, MI *origin_dt, MI *target_rank, MI *target_disp, MI *target_count, MI *target_dt, MI *win, MI *ie) { nqjf_put(origin_addr, origin_count, origin_dt, target_rank, target_disp, target_count, target_dt, win, ie); }
...
void mpi_recv        (void *buf, MI *count, MI *dt, MI *source, MI *tag, MI *comm, MI *status, MI *ie) { nqjf_recv(buf, count, dt, source, tag, comm, status, ie); }
...
void mpi_recv_init   (void *buf, MI *count, MI *dt, MI *source, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_recv_init(buf, count, dt, source, tag, comm, rq, ie); }
...
void mpi_reduce      (M3C void *sb, void *rb, MI *count, MI *dt, MI *op, MI *root, MI *comm, MI *ie) { nqjf_reduce(sb, rb, count, dt, op, root, comm, ie); }
...
void mpi_reduce_scatter  (M3C void *sb, void *rb, M3C MI *rcs, MI *dt, MI *op, MI *comm, MI *ie) { nqjf_reduce_scatter(sb, rb, rcs, dt, op, comm, ie); }
...
void mpi_request_free  (MI *rq, MI *ie) { nqjf_rq_free(rq, ie); }
...
void mpi_request_get_status  (MI *rq, int *flag, MI *status, MI *ie) { nqjf_rq_get_status(rq, flag, status, ie); }
...
void mpi_rsend       (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *ie) { nqjf_rsend(buf, count, dt, dest, tag, comm, ie); }
...
void mpi_rsend_init  (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_rsend_init(buf, count, dt, dest, tag, comm, rq, ie); }
...
void mpi_scan        (M3C void *sb, void *rb, MI *count, MI *dt, MI *op, MI *comm, MI *ie) { nqjf_scan(sb, rb, count, dt, op, comm, ie); }
...
void mpi_scatter     (M3C void *sb, MI *sc, MI *st, void *rb, MI *rc, MI *rt, MI *root, MI *comm, MI *ie) { nqjf_scatter(sb, sc, st, rb, rc, rt, root, comm, ie); }
...
void mpi_scatterv    (M3C void *sb, M3C MI *scs, M3C MI *displs, MI *st, void *rb, MI *rc, MI *rt, MI *root, MI *comm, MI *ie) { nqjf_scatterv(sb, scs, displs, st, rb, rc, rt, root, comm, ie); }
...
void mpi_send        (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *ie) { nqjf_send(buf, count, dt, dest, tag, comm, ie); }
...
void mpi_send_init   (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_send_init(buf, count, dt, dest, tag, comm, rq, ie); }
...
void mpi_sendrecv    (M3C void *sb, MI *sc, MI *st, MI *dest, MI *sg, void *rb, MI *rc, MI *rt, MI *source, MI *recvtag, MI *comm, MI *status, MI *ie) { nqjf_sendrecv(sb, sc, st, dest, sg, rb, rc, rt, source, recvtag, comm, status, ie); }
...
void mpi_sendrecv_replace(void *buf, MI *count, MI *dt, MI *dest, MI *sg, MI *source, MI *recvtag, MI *comm, MI *status, MI *ie) { nqjf_sendrecv_replace(buf, count, dt, dest, sg, source, recvtag, comm, status, ie); }
...
void mpi_ssend       (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *ie) { nqjf_ssend(buf, count, dt, dest, tag, comm, ie); }
...
void mpi_ssend_init  (M3C void *buf, MI *count, MI *dt, MI *dest, MI *tag, MI *comm, MI *rq, MI *ie) { nqjf_ssend_init(buf, count, dt, dest, tag, comm, rq, ie); }
...
void mpi_test        (MI *rq, MI *flag, MI *status, MI *ie) { nqjf_test(rq, flag, status, ie); }
...
void mpi_testall     (MI *count, MI *ar, MI *flag, MI *as, MI *ie) { nqjf_testall(count, ar, flag, as, ie); }
...
void mpi_testany     (MI *count, MI *ar, MI *index, MI *flag, MI *status, MI *ie) { nqjf_testany(count, ar, index, flag, status, ie); }
...
void mpi_testsome    (MI *incount, MI *ar, MI *outcount, MI *ai, MI *as, MI *ie) { nqjf_testsome(incount, ar, outcount, ai, as, ie); }
...
void mpi_wait        (MI *rq, MI *status, MI *ie) { nqjf_wait(rq, status, ie); }
...
void mpi_waitall     (MI *count, MI *ar, MI *as, MI *ie) { nqjf_waitall(count, ar, as, ie); }
...
void mpi_waitany     (MI *count, MI *rqs, MI *index, MI *status, MI *ie) { nqjf_waitany(count, rqs, index, status, ie); }
...
void mpi_waitsome    (MI *incount, MI *ar, MI *outcount, MI *ai, MI *as, MI *ie) { nqjf_waitsome(incount, ar, outcount, ai, as, ie); }
...
void mpi_win_complete  (MI *win, MI *ie) { nqjf_win_complete(win, ie); }
...
void mpi_win_create  (void *base, MPI_Aint *size, MI *disp_unit, MI *info, MI *comm, MI *win, MI *ie) { nqjf_win_create(base, size, disp_unit, info, comm, win, ie); }
...
void mpi_win_fence   (MI *assert, MI *win, MI *ie) { nqjf_win_fence(assert, win, ie); }
...
void mpi_win_free    (MI *win, MI *ie) { nqjf_win_free(win, ie); }
...
void mpi_win_post    (MI *group, MI *assert, MI *win, MI *ie) { nqjf_win_post(group, assert, win, ie); }
...
void mpi_win_start   (MI *group, MI *assert, MI *win, MI *ie) { nqjf_win_start(group, assert, win, ie); }
...
void mpi_win_wait    (MI *win, MI *ie) { nqjf_win_wait(win, ie); }
...
void mpi_iallgather  (M3C void *sb, MI *sc, MI *st, void *rb, MI *rc, MI *rt, MI *comm, MI *rq, MI *ie) { nqjf_iallgather(sb, sc, st, rb, rc, rt, comm, rq, ie); }
...
void mpi_iallgatherv  (M3C void *sb, MI *sc, MI *st, void *rb, M3C MI *rc, M3C MI *displs, MI *rt, MI *comm, MI *rq, MI *ie) { nqjf_iallgatherv(sb, sc, st, rb, rc, displs, rt, comm, rq, ie); }
...
void mpi_iallreduce  (M3C void *sb, void *rb, MI *count, MI *dt, MI *op, MI *comm, MI *rq, MI *ie) { nqjf_iallreduce(sb, rb, count, dt, op, comm, rq, ie); }
...
void mpi_ialltoall   (M3C void *sb, MI *sc, MI *st, void *rb, MI *rc, MI *rt, MI *comm, MI *rq, MI *ie) { nqjf_ialltoall(sb, sc, st, rb, rc, rt, comm, rq, ie); }
...
void mpi_ialltoallv  (M3C void *sb, M3C MI *scs, M3C MI *sdispls, MI *st, MI *rb, M3C MI *rcs, M3C MI *rdispls, MI *rt, MI *rq, MI *comm, MI *ie) { nqjf_ialltoallv(sb, scs, sdispls, st, rb, rcs, rdispls, rt, rq, comm, ie); }
...
void mpi_ibarrier    (MI *comm, MI *rq, MI *ie) { nqjf_ibarrier(comm, rq, ie); }
...
void mpi_ibcast      (void *buffer, MI *count, MI *dt, MI *root, MI *comm, MI *rq, MI *ie) { nqjf_ibcast(buffer, count, dt, root, comm, rq, ie); }
...
void mpi_igather     (M3C void *sb, MI *sc, MI *st, void *rb, MI *rc, MI *rt, MI *root, MI *comm, MI *rq, MI *ie) { nqjf_igather(sb, sc, st, rb, rc, rt, root, comm, rq, ie); }
...
void mpi_igatherv    (M3C void *sb, MI *sc, MI *st, void *rb, M3C MI *rcs, M3C MI *displs, MI *rt, MI *root, MI *comm, MI *rq, MI *ie) { nqjf_igatherv(sb, sc, st, rb, rcs, displs, rt, root, comm, rq, ie); }
...
void mpi_ireduce     (M3C void *sb, void *rb, MI *count, MI *dt, MI *op, MI *root, MI *comm, MI *rq, MI *ie) { nqjf_ireduce(sb, rb, count, dt, op, root, comm, rq, ie); }
...
void mpi_ireduce_scatter  (M3C void *sb, void *rb, M3C MI *rcs, MI *dt, MI *op, MI *comm, MI *rq, MI *ie) { nqjf_ireduce_scatter(sb, rb, rcs, dt, op, comm, rq, ie); }
...
void mpi_iscan       (M3C void *sb, void *rb, MI *count, MI *dt, MI *op, MI *comm, MI *rq, MI *ie) { nqjf_iscan(sb, rb, count, dt, op, comm, rq, ie); }
...
void mpi_iscatter    (M3C void *sb, MI *sc, MI *st, void *rb, MI *rc, MI *rt, MI *root, MI *comm, MI *rq, MI *ie) { nqjf_iscatter(sb, sc, st, rb, rc, rt, root, comm, rq, ie); }
...
void mpi_iscatterv   (M3C void *sb, M3C MI *scs, M3C MI *displs, MI *st, void *rb, MI *rc, MI *rt, MI *root, MI *comm, MI *rq, MI *ie) { nqjf_iscatterv(sb, scs, displs, st, rb, rc, rt, root, comm, rq, ie); }
